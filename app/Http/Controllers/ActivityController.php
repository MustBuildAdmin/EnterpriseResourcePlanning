...
class ActivityController extends Controller
{
    public function activity()
    {
        if (\Auth::user()->can("view CRM activity")) {
            $notes = Note::where("created_by", \Auth::user()->creatorId())
                ->orderBy("id", "desc")
                ->get();
            $tasks = Task::where("created_by", \Auth::user()->creatorId())
                ->orderBy("id", "desc")
                ->get();
            $emails = Email::where("created_by", \Auth::user()->creatorId())
                ->orderBy("id", "desc")
                ->get();
            $log_activities = LogActivity::where(
                "created_by",
                \Auth::user()->creatorId()
            )
                ->orderBy("id", "desc")
            ...
            foreach ($notes as $note) {
                $noteResult = Activity::get_activity(
                    $note->module_type,
                    $note->module_id
                );
                $noteResult["note"] = $note->note;
                $noteResult["created_at"] = $note->created_at->format(
                    "Y-m-d H:m:s"
                );
                $results[] = $noteResult;
            }
            foreach ($tasks as $task) {
                $taskResult = Activity::get_activity(
                    $task->module_type,
                    $task->module_id
                );
                $taskResult["note"] = $task->description;
                $taskResult["created_at"] = $task->created_at->format(
                    "Y-m-d H:m:s"
                );
            ...
            foreach ($emails as $email) {
                $emailResult = Activity::get_activity(
                    $email->module_type,
                    $email->module_id
                );
                $emailResult["note"] = $email->description;
                $emailResult["created_at"] = $email->created_at->format(
                    "Y-m-d H:m:s"
                );
            ...
            foreach ($log_activities as $log_activity) {
                $logactivityResult = Activity::get_activity(
                    $log_activity->module_type,
                    $log_activity->module_id
                );
                $logactivityResult["note"] = $log_activity->note;
                $logactivityResult[
                    "created_at"
                ] = $log_activity->created_at->format("Y-m-d H:m:s");
            ...
            foreach ($schedules as $schedule) {
                $scheduleResult = Activity::get_activity(
                    $schedule->module_type,
                    $schedule->module_id
                );
                $scheduleResult["note"] = $schedule->note;
                $scheduleResult["created_at"] = $schedule->created_at->format(
                    "Y-m-d H:m:s"
                );
            ...
            return redirect()
                ->back()
                ->with("errors", __("Permission denied."));
        }
    }

    public static function activity_store(
        $userid,
        $projectid,
        $logtype,
        $remark
