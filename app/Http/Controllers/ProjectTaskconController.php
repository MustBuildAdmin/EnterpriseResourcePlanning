...
class ProjectTaskconController extends Controller
{
    public function index($project_id)
    {
        ...
        if (\Auth::user()->can('manage project task')) {
            ...
            foreach ($stages as $status) {
                ...
                $task = Project_tasks_con::where('project_id', '=', $project_id);
            ...
            return view('project_task_con.index', compact('stages', 'stageClass', 'project'));
        } else {
    ...
    public function create($project_id, $stage_id)
    {
        if (\Auth::user()->can('create project task')) {
            $project = Construction_project::find($project_id);
            $hrs = Construction_project::projectHrs($project_id);

            return view('project_task_con.create', compact('project_id', 'stage_id', 'project', 'hrs'));
        } else {
            return redirect()->back()->with('error', __('Permission Denied.'));
        }
    }
    ...
    public function store(Request $request, $project_id, $stage_id)
    {
        ...
        if (\Auth::user()->can('create project task')) {
            ...
            $task = Project_tasks_con::create($post);
            ...
            $project_name = Construction_project::find($project_id);
            $project = Construction_project::where('id', $project_name->id)->first();
            if (isset($setting['task_notification']) && $setting['task_notification'] == 1) {
                $msg = $task->name.__('of').' '.$project->name.__(' created by').' '.\Auth::user()->name.'.';
    ...
    public function taskBoard($view)
    {
        ...
        if ($view == 'list') {
            ...
            $tasks = Project_tasks_con::where('created_by', \Auth::user()->creatorId())->get();

            return view('project_task_con.taskboard', compact('view', 'tasks'));
        } else {
            $tasks = Project_tasks_con::where('created_by', \Auth::user()->creatorId())->get();

            return view('project_task_con.grid', compact('tasks', 'view'));
        }
    ...
    public function taskboardView(Request $request)
    {
        ...
        if ($request->ajax() && $request->has('view') && $request->has('sort')) {
            ...
            $returnHTML = view('project_task_con.'.$request->view, compact('tasks'))->render();
    ...
    public function show($project_id, $task_id)
    {
        ...
        if (\Auth::user()->can('view project task')) {
            $allow_progress = Construction_project::find($project_id)->task_progress;
            $task = Project_tasks_con::find($task_id);

            return view('project_task_con.view', compact('task', 'allow_progress'));
        } else {
            return redirect()->back()->with('error', __('Permission Denied.'));
        }
    }
    ...
    public function edit($project_id, $task_id)
    {
        if (\Auth::user()->can('edit project task')) {
            ...
            return view('project_task_con.edit', compact('project', 'task', 'hrs'));
        } else {
            return redirect()->back()->with('error', __('Permission Denied.'));
        }
    }
    ...
    public function destroy($project_id, $task_id)
    {
        ...
        if (\Auth::user()->can('delete project task')) {
            Project_tasks_con::deleteTask([$task_id]);
    ...
    public function checklistStore(Request $request, $projectID, $taskID)
    {
        ...
        if (\Auth::user()->can('view project task')) {
            ...
            $checkList = TaskChecklistcon::create($post);
            $user = $checkList->user;
    ...
    public function checklistUpdate($projectID, $checklistID)
    {
        ...
        if (\Auth::user()->can('view project task')) {
            $checkList = TaskChecklistcon::find($checklistID);
        ...
        if (\Auth::user()->can('view project task')) {
            ...
            if (isset($request->sort)) {
                foreach ($request->sort as $index => $taskID) {
                    if (! empty($taskID)) {
                        echo $index.'-'.$taskID;
                        $task = Project_tasks_con::find($taskID);
    ...
    public function taskGet($task_id)
    {
        if (\Auth::user()->can('view project task')) {
            ...
            $html .= '<span class="badge badge-pill badge-xs badge-'.Project_tasks_con::$priority_color[$task->priority].'">'.Project_tasks_con::$priority[$task->priority].'</span>';
            $html .= '</div>';
