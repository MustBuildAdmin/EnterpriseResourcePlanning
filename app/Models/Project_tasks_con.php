...
class Project_tasks_con extends Model
{
    ...
    public static function deleteTask($task_ids)
    {
        ...
        foreach($task_ids as $key => $task_id)
        {
            ...
            if($task)
            {
                ...
                // Delete Checklists
                TaskChecklistcon::where('task_id', '=', $task->id)->delete();

                // Delete Comments
                TaskCommentcon::where('task_id', '=', $task->id)->delete();
    ...
    {
        return Activity_logs_con::where('user_id', '=', \Auth::user()->id)->where('project_id', '=', $this->project_id)->where('task_id', '=', $this->id)->get();
    }
