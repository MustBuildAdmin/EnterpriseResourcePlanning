...
@section('page-title')
    {{ucwords($project->name).__("'s Expenses")}}
@endsection
...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item">{{ucwords($project->name)}}</li>
        <!-- <a href="{{route('construction_project.show',$project->id)}}">    {{ucwords($project->name)}}</a> -->
    <li class="breadcrumb-item">{{ucwords($project->name).__("'s Expenses")}}</li>
...
@section('action-btn')
    <div class="float-end">
        @can('create expense')
            <a href="#" class="btn btn-primary btn-sm" data-url="{{ route('projectscon.expenses.create',$project->id) }}" data-ajax-popup="true" data-bs-toggle="tooltip" title="{{__('Create')}}" data-size="lg" data-title="{{__('Create Expense')}}">
                <span class="btn-inner--icon"><i class="ti ti-plus"></i></span>
            </a>
        @endcan
        <a href="{{ route('construction_project.show',$project->id) }}" class="btn btn-primary btn-sm" data-bs-toggle="tooltip" title="{{__('Back')}}">
                                            ...
                                            <td class="text-end w-15">
                                                <div class="actions">
                                                    @can('edit expense')
                                                        ...
                                                        <a href="#" class="mx-3 btn btn-sm align-items-center" data-url="{{ route('projectscon.expenses.edit',[$project->id,$expense->id]) }}" data-ajax-popup="true" data-size="lg" data-title="{{__('Edit ').$expense->name}}" data-bs-toggle="tooltip" title="{{__('Edit')}}" data-original-title="Edit">
                                                    ...
                                                    @can('delete expense')
                                                            <div class="action-btn bg-danger ms-2">
                                                                {!! Form::open(['method' => 'DELETE', 'route' => ['projectscon.expenses.destroy',$expense->id],'id'=>'delete-expense-'.$expense->id]) !!}
