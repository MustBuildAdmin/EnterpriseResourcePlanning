...
<input type="submit" id="edit_job_stage" value="{{__('Update')}}" class="btn btn-primary submit_button">
</div>
{{Form::close()}}
    ...
    $(document).ready(function(){
        $(document).on("keyup", '.get_name', function () {
            ...
            $.ajax({
                ...
                type : 'GET',
                data : { 'get_id': "{{$jobStage->id}}", 'get_name' : $(".get_name").val(), 'form_name' : "JobStage" },
        ...
        $(document).on('submit', 'form', function() {
            $('#edit_job_stage').attr('disabled', 'disabled');
        });
    });
</script>