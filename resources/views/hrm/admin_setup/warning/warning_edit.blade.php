     ...
     <div class="row">
        @if(\Auth::user()->type != 'employee')
            <div class="form-group col-md-6 col-lg-6">
                ...
                {{ Form::select('warning_by', $employees,null, array('class' => 'form-select','required'=>'required','placeholder'=>'Select Employee')) }}
            </div>
        @endif
        <div class="form-group col-lg-6 col-md-6">
            {{Form::label('warning_to',__('Warning To*'),['class'=>'form-label'])}}
            {{Form::select('warning_to',$employees,null,array('class'=>'form-select','required'=>'required','placeholder'=>'Select Employee'))}}
        </div>
        <div class="form-group col-lg-6 col-md-6">
...
<div class="modal-footer">
    ...
    <input type="submit" id="edit_warning" value="{{__('Update')}}" class="btn  btn-primary">
</div>
{{Form::close()}}
<script> 
    $(document).ready(function(){
        $(document).on('submit', 'form', function() {
            $('#edit_warning').attr('disabled', 'disabled');
        });
    });
</script>
