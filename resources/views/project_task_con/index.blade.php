...
@section('page-title')
    {{ucwords($project->project_name).__("Tasks")}}
@endsection
...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item"><a href="{{route('productivity_show',$project->id)}}">{{ucwords($project->name)}}</a></li>
                        ...
                        <div class="card">
                            <div class="card-header">
                                @can('create project task')
                                    <div class="float-end">
                                        <a href="#" data-size="lg" data-url="{{ route('projectscon.tasks.create',[$project->id,$stage->id]) }}" data-ajax-popup="true" data-bs-toggle="tooltip" title="{{__('Add Task in ').$stage->name}}" class="btn btn-sm btn-primary">
                            ...
                            <div class="card-body kanban-box" id="task-list-{{$stage->id}}" data-status="{{$stage->id}}">
                                @foreach($tasks as $taskDetail)
                                    <div class="card draggable-item" id="{{$taskDetail->id}}">
                                        <div class="pt-3 ps-3">
                                            <div class="badge-xs badge bg-{{\App\Models\Project_tasks_con::$priority_color[$taskDetail->priority]}} p-2 px-3 rounded">{{ __(\App\Models\Project_tasks_con::$priority[$taskDetail->priority]) }}</div>
                                        </div>
                                        <div class="card-header border-0 pb-0 position-relative">
                                            <h5>
                                                <a href="#" data-url="{{ route('projectscon.tasks.show',[$project->id,$taskDetail->id]) }}" data-ajax-popup="true" data-size="lg" data-bs-original-title="{{$taskDetail->name}}">{{$taskDetail->name}}</a>
                                            </h5>
                                                ...
                                                <div class="btn-group card-option">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-end">
                                                        ...
                                                        @can('view project task')
                                                            <a href="#!" data-size="md" data-url="{{ route('projectscon.tasks.show',[$project->id,$taskDetail->id]) }}" data-ajax-popup="true" class="dropdown-item" data-bs-original-title="{{__('View')}}">
                                                                <i class="ti ti-bookmark"></i>
                                                                <span>{{__('View')}}</span>
                                                            </a>
                                                        @endcan
                                                        @can('edit project task')
                                                            <a href="#!" data-size="lg" data-url="{{ route('projectscon.tasks.edit',[$project->id,$taskDetail->id]) }}" data-ajax-popup="true" class="dropdown-item" data-bs-original-title="{{__('Edit ').$taskDetail->name}}">
                                                                <i class="ti ti-pencil"></i>
                                                                <span>{{__('Edit')}}</span>
                                                            </a>
                                                        @endcan
                                                        @can('delete project task')
                                                            {!! Form::open(['method' => 'DELETE', 'route' => ['projectscon.tasks.destroy', [$project->id,$taskDetail->id]]]) !!}
