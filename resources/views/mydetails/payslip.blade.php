@include('new_layouts.header')
    ...
    <div class="table-responsive mt-4">
        <table id="pc-dt-render-column-cells" class="table card-table table-vcenter text-nowrap datatable">
            <thead>
                <tr>
                    {{-- <th>{{ __('Id') }}</th> --}}
...
<script>
    $(document).ready(function () {
        ...
        function callback() {
            ...
            $.ajax({
                url: '{{ route('payslip.search_json1') }}',
                type: 'POST',
                data: {
                        ...
                        $.each(data, function (indexInArray, valueOfElement) {
                            ...
                            if (valueOfElement[7] != 0) {
                                var payslip =
                                    ...
                                    id +
                                    '/' + datePicker +
                                    '" data-size="lg"  data-ajax-popup="true" class=" text-white" data-title="{{ __('Employee Payslip') }}">' +
                                    '{{ __('Payslip') }}' + '</a> ';
                            }
                            ...
                            tr +=
                                ...
                                '<td>' + status + '</td>' +
                                '<td><div class="badge bg-danger p-2 px-3 rounded green">' + payslip + '</div></td>' +
                                '</tr>';
                        });
                    } else {
