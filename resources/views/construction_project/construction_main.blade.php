                    ...
                    <div class="col-8">
                        <div class="mb-3">
                            <div class="row g-2">
                                ...
                                <div class="col-auto">
                                    <a href="javascript:void(0)" id="searchbtnproj" class="btn btn-icon"
                                        aria-label="Button">
                    ...
                    @can('create project')
                        <div class="col-4">
                            <a href="#" data-size="xl" data-url="{{ route('projects.create') }}"
                                data-ajax-popup="true" data-title="{{ __('Create New Project') }}" data-bs-toggle="tooltip"
                                title="{{ __('Create New Project') }}" class="btn btn-primary w-100" data-bs-toggle="modal"
                                data-bs-target="#invite-Sub-Contractor">
                                {{ __('Create a Project') }}
                            </a>
                        </div>
                    @endcan
                        ...
                        <div class="card">
                            ...
                            <div class="card-body p-4 py-5 text-center">
                                ...
                                @if ($project_image != 0 && $project_image != '')
                                    ...
                                    @endphp
                                    <img id="image" src="{{ $image }}" class="avatar avatar-xl mb-4 rounded"
                                        alt="">
                                @else
                                ...
                                <h3 class="mb-0">
                                    @php
                                        $project_instances = \App\Models\Instance::where('project_id', $project->id)->get();
                                    ...
                                    @else
                                        <a class="text-dark text-capitalize" data-size="lg"
                                            href="{{ route('projects.instance_project', [$project_instances[0]['id'], $project->id, 'Base']) }}"
                                            data-bs-toggle="tooltip">{{ $project->project_name }}{{ $project->project_id }}
                                        </a>
                                    @endif
                                </h3>
                                <p class="text-secondary mb-0">Start Date:
                                    {{ Utility::getDateFormated($project->start_date) }}</p>
                                <p class="text-secondary">End Date: {{ Utility::getDateFormated($project->end_date) }}
                                </p>
                                <p class="mb-3">
                                    @php
                                        if ($project->status != '') {
                                ...
                                <div>
                                    <div class="avatar-list avatar-list-stacked">
                                        ...
                                        @endphp
                                        @if (isset($projectmembers) && !empty($projectmembers) && count($projectmembers) > 0)
                                                    ...
                                                    @if ($name_r->avatar)
                                                        <a href="#" class="avatar avatar-sm rounded">
                                                            <img src="{{ !empty(\Auth::user()->avatar)
                                                                ? $profile . $name_r->avatar
                                                                : asset(Storage::url('uploads/avatar/avatar.png')) }}"
                                                                alt="{{ strtoupper($short) }}"
                                                                data-bs-toggle="tooltip" title="{{ $name_r->name }}"
    ...
    @else
        <div class="page-body">
            <div class="container-xl d-flex flex-column justify-content-center">
                <div class="empty">
                    <div class="empty-img">
                        <img src="{{ asset('assets/images/undraw_printing_invoices_5r4r.svg') }}" height="128"
                            alt="">
                    </div>
