   ...
   <div class="col-md-6">
      ...
      <div class="float-end">
            <div class="float-right ">
                @can('create expense')
                    <a href="#" class="btn btn-outline-primary w-20 topbtn" data-url="{{ route('projects.expenses.create',$project->id) }}" data-ajax-popup="true" data-bs-toggle="tooltip" title="{{__('Create')}}" data-size="lg" data-title="{{__('Create Expense')}}">
                        <span class="btn-inner--icon"><i class="ti ti-plus"></i></span>
                    </a>
                @endcan
                <a href="{{ route('projects.show',$project->id) }}" class="btn btn-outline-primary w-20 topbtn" data-bs-toggle="tooltip" title="{{__('Back')}}">
                            ...
                            @if(isset($project->expense) && !empty($project->expense) && count($project->expense) > 0)
                                @foreach($project->expense as $expense)
                                    <tr>
                                        ...
                                        <td>
                                            <span class="h6 font-weight-bold mb-0">{{ $expense->name }}</span>
                                            ...
                                            <td class="text-end w-15">
                                                <div class="actions">
                                                        ...
                                                        {!! Form::open(['method' => 'DELETE', 'route' => ['projects.expenses.destroy',$expense->id],'id'=>'delete-expense-'.$expense->id]) !!}
                                                            ...
                                                            @can('edit expense')
                                                                <a href="#" class="mx-3 btn btn-sm align-items-center backgroundnone" data-url="{{ route('projects.expenses.edit',[$project->id,$expense->id]) }}" data-ajax-popup="true" data-size="lg" data-title="{{__('Edit ').$expense->name}}" data-bs-toggle="tooltip" title="{{__('Edit')}}" data-original-title="Edit">
