    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $(document).on('click', '.invite_usr', function () {
                ...
                $.ajax({
                    url: '{{ route('invite-cons_project-user-member') }}',
                    method: 'POST',
                    dataType: 'json',
                    data: {
...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item"><a href="{{route('productivity')}}">{{__('Project')}}</a></li>
...
@section('action-btn')
    <div class="float-end">
        ...
        @if(\Auth::user()->type!='client' || (\Auth::user()->type=='client' ))
            <a href="{{ route('projecttimecon.tracker',$project->id) }}" class="btn btn-sm btn-primary">
                {{__('Tracker')}}
            </a>
        @endif
        @can('view expense')
        ...
        @can('create project task')
            <a href="{{ route('projectscon.tasks.index',$project->id) }}" class="btn btn-sm btn-primary">
                {{__('Task')}}
            </a>
        @endcan
        {{-- @can('edit project')
            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex align-items-center justify-content-between">
                        ...
                        @can('edit project')
                            <div class="float-end">
                                <a href="#" data-size="lg" data-url="{{ route('invite-cons_project-member', $project->id) }}" data-ajax-popup="true" data-bs-toggle="tooltip" title="" class="btn btn-sm btn-primary" data-bs-original-title="{{__('Add Member')}}">
                ...
                <div class="card-header">
                    @can('create milestone')
                        <div class="d-flex align-items-center justify-content-between">
                            ...
                            <div class="float-end">
                                <a href="#" data-size="md" data-url="{{ route('con.project.milestone', $project->id) }}" data-ajax-popup="true" data-bs-toggle="tooltip" title="" class="btn btn-sm btn-primary" data-bs-original-title="{{__('Create New Milestone')}}">
                                    <i class="ti ti-plus"></i>
                                </a>
                            </div>
                        </div>
                ...
                <div class="card-body">
                    <ul class="list-group list-group-flush">
                        @if($project->milestones->count() > 0)
                            @foreach($project->milestones as $milestone_con)
                                <li class="list-group-item px-0">
                                    <div class="row align-items-center justify-content-between">
                                        <div class="col-sm-auto mb-3 mb-sm-0">
                                            <div class="d-flex align-items-center">
                                                <div class="div">
                                                    <h6 class="m-0">{{ $milestone_con->title }}
                                                        <span class="badge-xs badge bg-{{\App\Models\Project::$status_color[$milestone_con->status]}} p-2 px-3 rounded">{{ __(\App\Models\Project::$project_status[$milestone_con->status]) }}</span>
                                                    </h6>
                                                    <small class="text-muted">{{ $milestone_con->tasks->count().' '. __('Tasks') }}</small>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="col-sm-auto text-sm-end d-flex align-items-center">
                                            <div class="action-btn bg-warning ms-2">
                                                <a href="#" data-size="md" data-url="{{ route('con_project.milestone.show',$milestone_con->id) }}" data-ajax-popup="true" data-bs-toggle="tooltip" title="{{__('View')}}" class="btn btn-sm">
                                                    <i class="ti ti-eye text-white"></i>
                                                </a>
                                            </div>
                                            <div class="action-btn bg-info ms-2">
                                                <a href="#" data-size="md" data-url="{{ route('con_project.milestone.edit',$milestone_con->id) }}" data-ajax-popup="true" data-bs-toggle="tooltip" title="{{__('Edit')}}" class="btn btn-sm">
                                                    <i class="ti ti-pencil text-white"></i>
                                                </a>
                                            </div>
                                            <div class="action-btn bg-danger ms-2">
                                                {!! Form::open(['method' => 'DELETE', 'route' => ['con_project.milestone.destroy', $milestone_con->id]]) !!}
